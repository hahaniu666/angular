...
(function () {
    angular.module('qh-test-front')
        .config(['$stateProvider', function ($stateProvider) {
            ...
            $stateProvider.state("main.clean", {
                url: "/clean?myjson&title",
                views: {
                    "@": {
                        templateUrl: 'views/main/clean/index.root.html',
    ...
    // ----------------------------------------------------------------------------
    cleanController.$inject = ['$scope', '$http', '$state', 'appConfig', 'imgService', '$httpParamSerializer', '$mdBottomSheet', 'alertService', 'orderService'];
    function cleanController($scope, $http, $state, appConfig, imgService, $httpParamSerializer, $mdBottomSheet, alertService, orderService) {
        $scope.pageSize = 20;
                ...
                for (var j = 0; j < $scope.list.length; j++) {
                    for (var l = 0; l < $scope.list[j].children.length; l++) {
                        if ($scope.list[j].children[l].code === c_info) {
                            ...
                            $scope.list[j].children[l].service_all = [];
                            $scope.list[j].check = true;
                            $scope.list[j].children[l].check = true;
                            ...
                            for (var i = 0; i < $scope.list[j].children[l].service_item.length; i++) {
                                for (var k = 0; k < $scope.list[j].children[l].service_item[i].sku.length; k++) {
                                    ...
                                    $scope.list[j].children[l].service_all.push($scope._tempsku);
        ...
        //添加到洗衣篮 cl:单个sku l:list的所有,用来匹配加入到那个层的value中 s:item
