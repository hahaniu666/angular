...
(function () {
    angular.module('qh-test-front').config(['$stateProvider', function ($stateProvider) {
         ...
         * TODO 商品详情，主状态。
         *!/
        $stateProvider.state("main.item.itemCopy", {
            url: '/itemCopy?itemId&itemName&s&skuId&type',        //type判断是否为学生租赁的商品详情显示
            resolve: {
                curUser: ['userService', function (userService) {
                    return userService.getCurUser(false, false);
                }]
            },
            views: {
                "@": {
                    templateUrl: 'views/main/item/itemCopy.html',
                    controller: itemCopyController,
                    controllerAs: 'vm'
                }
            }
        });
    }]);
    itemCopyController.$inject = ['$window', '$scope', '$http', '$state', '$rootScope', '$interval', '$filter', '$timeout', 'appConfig', 'imgService', "curUser", '$mdBottomSheet', "alertService"];
    function itemCopyController($window, $scope, $http, $state, $rootScope, $interval, $filter, $timeout, appConfig, imgService, curUser, $mdBottomSheet, alertService) {
        var vm = this;
        ////返回顶部
        ...
        $scope.queryItemDetail = function () {
            $http.get(appConfig.apiPath + "/item/detail?itemId=" + $scope.itemId)
                .success(function (data) {
        ...
        $scope.itemPayBuy = function (status) {
            ...
            if (status === 0) {
                ...
                // 不是租赁的时候才进行判断库存是否充值
                // 获取该商品的属性
                $http.get(appConfig.apiPath + "/qhOrder/check?sku=" + $scope.checkSku.sku + "&num=" + $scope.skuProp.skuNum + "&day=" + $scope.skuProp.day)
                    .then(function (resp) {
