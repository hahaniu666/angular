(function () {
    ...
    angular.module('qh-test-front').config(['$stateProvider', function ($stateProvider) {
         ...
         * 订单初次申请退货
         */
        $stateProvider.state("main.order.orgRefund", {
            url: '/orgRefund?no&orderId&type&sku',
            views: {
                "@": {
                    templateUrl: 'views/main/order/orgRefund/index.root.html',
    ...
    function refundController($scope, $http, $state, $httpParamSerializer, alertService, appConfig, imgService, wxService, FileUploader) {
        ...
        $scope.refund={};
        $scope.refund.orderId =$state.params.orderId;
        $scope.refund.id = $state.params.no;
        $scope.orderStatus = "";
        ...
        $http({
            method: 'GET',
            url: appConfig.apiPath + "/orgOrder/orderItemDetail?orderId=" + $state.params.orderId + "&no=" + $state.params.no
        }).then(function (resp) {
            var data = resp.data;
            $scope.refund = data;
        ...
        $scope.initRefundType = function () {
            if ($scope.orderStatus === 'UNSHIPPED') {
                $scope.patterns = 0;
                $scope.refundType = [{
                    name: '仅退款',
                    value: 'ORGORDER'
        ...
        $scope.changeType = function () {
            if (vm.type === "ORGORDER_SKU") {
                $scope.patterns = 1;
            } else if (vm.type === "ORGORDER") {
                $scope.patterns = 0;
            }
        };
                ...
                if (data) {
                    ...
                    $http({
                        ...
                        data: {
                            ...
                            skuId: $state.params.sku,
                            // no: $scope.refund.no,
                            price: $scope.refund.price,
                            // imgs: imgsTemp,
                            memo: vm.memo
