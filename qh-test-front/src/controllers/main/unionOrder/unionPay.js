(function () {
    ...
    angular.module('qh-test-front').config(['$stateProvider', function ($stateProvider) {
         ...
         * 订单付款
         */
        $stateProvider.state("main.unionOrder.unionPay", {
            url: '/unionPay?orderId&id&payType&payId', // orderId代表的是qhPay的支付Id.id只有在详情页面进来的时候才有值,payType支付方式
            views: {
                "@": {
                    templateUrl: 'views/main/unionOrder/unionPay/index.root.html',
                    controller: orderPayController
                }
            }
        });
    }]);
    orderPayController.$inject = ['alertService', '$httpParamSerializer', '$scope', '$http', '$state', '$log',
        '$timeout', '$rootScope', '$interval', 'appConfig', '$mdDialog', '$mdBottomSheet'];
    function orderPayController(alertService, $httpParamSerializer, $scope, $http, $state, $log,
                                $timeout, $rootScope, $interval, appConfig, $mdDialog, $mdBottomSheet) {
        $scope.list = [
            {
                name: '支付宝支付',
                        ...
                        $http({
                            ...
                            if (data.code === 'NOT_WEIXIN') {
                                ...
                                window.Wechat.isInstalled(function () {
                                    ...
                                    alertService.msgAlert("cancle", "您尚未安装微信，无法使用微信登录");
                                });
                            } else if (data.code === "AMOUNT_ZERO") {
                                $timeout(function () {
